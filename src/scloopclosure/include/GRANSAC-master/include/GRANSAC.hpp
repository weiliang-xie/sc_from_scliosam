...
{
	// T - AbstractModel
	template <class T, int t_NumParams>		//t_NumParams指用于求解ransac的最少点云数量
	class RANSAC
	{
	private:
	...
	public:
		...
		bool Estimate(const std::vector<std::shared_ptr<AbstractParameter>> &Data)
		{
			...
			//定义一个均匀分布的区间（0 ~ datasie-1）后续使用该函数？变量？可以输出合适的整数
...
#pragma omp parallel for
			for (int i = 0; i < m_MaxIterations; ++i)
			{
				...
				//随机重排留下的数据（目前是全部） 重排后将前t_NumParams个复制到ransac的容器中
				...
				m_SampledModels[i] = RandomModel;	//存入
			}

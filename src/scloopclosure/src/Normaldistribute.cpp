...
std::pair<int, float> NDManager::NDdetectLoopClosureID ( void )
{
    ...
    if( (int)polarcontext_invkeys_mat_ca.size() < ND_NUM_EXCLUDE_RECENT + 1 ) //储存的描述符数量是否足够
    {
     ...
     */
    TicToc t_calc_dist;   
    for ( int candidate_iter_idx = 0; candidate_iter_idx < ND_NUM_CANDIDATES_FROM_TREE*2; candidate_iter_idx++ )
    {
